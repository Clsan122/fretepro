...
const FreightSelection: React.FC = () => {
  ...
  const handleSelectByClient = (clientId: string) => {
    const clientFreights = freights.filter(freight => freight.clientId === clientId).map(freight => freight.id);
  ...
  return <Layout>
      <div className="container mx-auto py-6 pb-20">
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon" onClick={() => navigate('/freights')}>
                <ArrowLeft className="h-4 w-4" />
              </Button>
              <h1 className="text-2xl font-bold tracking-tight">Selecionar Fretes para Recibo</h1>
            </div>
            
            <Button onClick={handleGenerateMultipleReceipt} disabled={selectedFreights.length === 0} className="gap-2">
        ...
        {loading ? <div className="flex justify-center py-8">
          ...
          </div> : freights.length === 0 ? <Card className="p-8 text-center">
            <p className="text-muted-foreground">Nenhum frete encontrado.</p>
            <Button onClick={() => navigate('/freights')} variant="outline" className="mt-4">
              Voltar para a lista de fretes
            </Button>
          </Card> : <div className="space-y-6">
            <div className="flex justify-between items-center bg-muted p-4 rounded-lg">
              <div className="flex items-center space-x-2">
                <Checkbox id="select-all" checked={selectedFreights.length === freights.length && freights.length > 0} onCheckedChange={handleSelectAll} />
          ...
          return <div key={clientId} className="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
                  <div className="p-4 border-b flex justify-between items-center">
                    <div className="flex items-center space-x-2">
                      <Checkbox id={`client-${clientId}`} checked={clientFreights.every(f => selectedFreights.includes(f.id))} onCheckedChange={() => handleSelectByClient(clientId)} />
                  ...
                  <div className="overflow-x-auto">
                    <Table>
                      ...
                      <TableBody>
                        {clientFreights.map(freight => <TableRow key={freight.id} className={selectedFreights.includes(freight.id) ? "bg-primary/5" : ""}>
                            <TableCell>
                              <Checkbox checked={selectedFreights.includes(freight.id)} onCheckedChange={() => handleCheckboxChange(freight.id)} />
                            </TableCell>
