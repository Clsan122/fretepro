...
export const QuotationPdfDocument: React.FC<QuotationPdfDocumentProps> = ({ quotation }) => {
  return (
    <div id="quotation-pdf" className="bg-white p-8 max-w-3xl mx-auto font-sans">
      ...
      <div className="flex justify-between items-start mb-4 border-b pb-2 print-tight-margins">
        <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3 bg-freight-50 p-3 rounded-lg print-tight-margins">
        <div>
          <h2 className="text-sm font-semibold mb-1 text-freight-800">Origem</h2>
          <p className="text-freight-700 font-medium text-sm">{quotation.originCity} / {quotation.originState}</p>
        </div>
        <div>
          <h2 className="text-sm font-semibold mb-1 text-freight-800">Destino</h2>
          <p className="text-freight-700 font-medium text-sm">{quotation.destinationCity} / {quotation.destinationState}</p>
        </div>
      </div>
      
      {/* Detalhes da carga */}
      <div className="mb-3 print-tight-margins">
        <h2 className="text-sm font-semibold mb-2 text-freight-800 border-b border-freight-200 pb-1">Detalhes da Carga</h2>
          ...
          <table className="w-full text-xs">
            <tbody>
              <tr className="border-b border-freight-200 bg-freight-50">
                <td className="py-1 px-2 font-medium text-freight-700">Volumes:</td>
                <td className="py-1 px-2 text-freight-800">{quotation.volumes}</td>
                <td className="py-1 px-2 font-medium text-freight-700">Peso:</td>
                <td className="py-1 px-2 text-freight-800">{quotation.weight} kg</td>
              </tr>
        ...
        {quotation.measurements && quotation.measurements.length > 0 && (
          <div className="mt-2">
            <h3 className="text-xs font-medium mb-1 text-freight-700">Dimensões:</h3>
            <div className="overflow-auto">
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="bg-freight-100">
                    <th className="py-1 px-2 text-left text-xs">Comp. (cm)</th>
                    <th className="py-1 px-2 text-left text-xs">Larg. (cm)</th>
                    <th className="py-1 px-2 text-left text-xs">Alt. (cm)</th>
                    <th className="py-1 px-2 text-left text-xs">Qtd</th>
                  </tr>
                </thead>
                <tbody>
      ...
      {/* Composição do Frete */}
      <div className="mb-3 print-tight-margins">
        <h2 className="text-sm font-semibold mb-2 text-freight-800 border-b border-freight-200 pb-1">Composição do Frete</h2>
        <div className="bg-white border border-freight-200 rounded-lg overflow-hidden">
          <table className="w-full text-xs">
            <tbody>
              <tr className="border-b border-freight-200">
                <td className="py-1 px-2 font-medium text-freight-700">Valor do Frete:</td>
                <td className="py-1 px-2 text-freight-800 text-right">{formatCurrency(quotation.freightValue)}</td>
              </tr>
              <tr className="border-b border-freight-200 bg-freight-50">
              ...
              <tr className="bg-freight-700 text-white">
                <td className="py-2 px-2 font-bold">VALOR TOTAL:</td>
                <td className="py-2 px-2 font-bold text-right">{formatCurrency(quotation.totalValue)}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      {/* Observações */}
      {quotation.notes && (
        <div className="mb-3 bg-freight-50 p-2 rounded-lg print-tight-margins">
          <h2 className="text-sm font-semibold mb-1 text-freight-800">Observações</h2>
          <p className="text-freight-700 whitespace-pre-wrap text-xs">{quotation.notes}</p>
        </div>
      )}
      
      {/* Rodapé */}
      <div className="mt-3 pt-2 border-t border-freight-200 text-center text-xs text-freight-500">
        ...
        <div className="mt-2 text-xs text-freight-400">
